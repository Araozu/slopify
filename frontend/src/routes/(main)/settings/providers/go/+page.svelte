<script lang="ts">
	...
	import { createGoKey, deleteGoKey } from '$lib/go-key-client';
	import { invalidateGoKeys, goKeysQueryOptions } from '$lib/queries/go-key-query';
	import type { GoApiKey } from '$lib/types';

	const queryClient = useQueryClient();
	const keysQuery = createQuery(() => goKeysQueryOptions());

	const keys = $derived((keysQuery.data ?? []) as GoApiKey[]);
	...
	const createMut = createMutation(() => ({
		mutationFn: ({ name, apiKey }: { name: string; apiKey: string }) =>
			createGoKey({ name, apiKey }),
		onSuccess: async () => {
			newName = '';
			newApiKey = '';
			formError = '';
			await invalidateGoKeys(queryClient);
		}
	}));

	const deleteMut = createMutation(() => ({
		mutationFn: (keyId: string) => deleteGoKey(keyId),
		onSuccess: async () => {
			await invalidateGoKeys(queryClient);
		}
	}));
...
<div class="flex h-[calc(100vh-2rem)] min-h-0 w-full flex-col bg-background">
	<header class="flex items-center border-b bg-background/20 px-4 py-1 backdrop-blur-xl md:px-6">
		<div class="flex items-center gap-2">
			...
			<h1 class="text-sm font-bold tracking-tight">OpenCode Go</h1>
		</div>
	</header>
			...
			<section class="space-y-4">
				<div class="space-y-1">
					...
					<p class="max-w-2xl text-xs text-muted-foreground/60">
						Add your OpenCode Go subscription API keys. Go is a curated subset of models — Kimi,
						GLM, DeepSeek, Qwen, MiniMax, and more through a single API.
					</p>
				</div>
				...
				<div class="grid gap-4 sm:grid-cols-2">
					<div class="rounded-xl border bg-muted/30 p-4 shadow-inner ring-1 ring-border/50">
						...
						<p class="mt-2 text-xs leading-relaxed text-foreground/60">
							Kimi, GLM, DeepSeek, GPT, Grok, and other models are routed through the
					...
					<div class="rounded-xl border bg-muted/30 p-4 shadow-inner ring-1 ring-border/50">
						...
						<p class="mt-2 text-xs leading-relaxed text-foreground/60">
							Qwen (<span class="font-mono">qwen*</span>) and MiniMax (<span class="font-mono"
								>minimax*</span
							>) models are routed through the Anthropic Messages endpoint. A leading
							<span class="font-mono">opencode-go/</span> prefix is stripped automatically.
						</p>
					</div>
				</div>
			</section>
						...
						{:else if keys.length === 0}
							...
							>
								...
								<p class="mt-1 text-xs text-muted-foreground/50">
									Add your first Go API key to get started.
								</p>
							</div>
						{:else}
